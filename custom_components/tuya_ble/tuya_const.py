"""Tuya cloud constants vendored from the Home Assistant Tuya integration.

Home Assistant Core removed these from `homeassistant.components.tuya.const`
in 2024.2, when the Tuya integration migrated to the device sharing SDK.
Tuya BLE still authenticates against the Tuya IoT platform with a custom
project (access ID / secret), so the constants are kept here instead.
